...
forModelling2016 <- read_csv("253279_41.53_-115.50_2016.csv")
forModelling2017 <- read_csv("253279_41.53_-115.50_2017.csv")
forModelling2018 <- read_csv("253279_41.53_-115.50_2018.csv")
forModelling2019 <- read_csv("253279_41.53_-115.50_2019.csv")
forModelling2020 <- read_csv("253279_41.53_-115.50_2020.csv")
forModelling2021 <- read_csv("253279_41.53_-115.50_2021.csv")
rainfall <-  read_excel("Rainfall_San Fran 2016-2023.xlsx")
...
forModelling %>%
  ...
  bind_cols(forModelling %>%
              filter(Date >= "2018-07-14") %>%
              filter(Date <= "2018-11-30") %>%
              select(Location, overall_fire_impact, log10Phytoplankton) %>%
              bind_rows(forModelling %>%
                          filter(Date >= "2018-07-14") %>%
                          filter(Date <= "2018-11-30") %>%
                          select(Location, overall_fire_impact, log10Phytoplankton) %>%
                          mutate(overall_fire_impact = 'zero'))) %>%
  pivot_longer(c(-Location, -overall_fire_impact, -log10Phytoplankton)) %>%
  mutate(overall_fire_impact = factor(overall_fire_impact, levels = c("high", "med", "low", "zero"))) %>%
  select(-name) %>%
  pivot_longer(c(-Location, -overall_fire_impact)) %>%
...
forModelling %>%
  ...
  median_qi(value) %>%
  mutate(`Bushfire Impact` = ifelse(overall_fire_impact == "high", "High", "Zero")) %>%
  ...
  scale_fill_manual(values = c("#B31529", "white")) +
...
predictions <- forModelling %>%
  ...
  bind_cols(forModelling %>%
              ...
              bind_rows(forModelling %>%
                          ...
                          mutate(overall_fire_impact = 'zero')) %>%
              mutate(id = row_number())) %>%
  pivot_longer(c(-overall_fire_impact, -log10Phytoplankton, -id)) %>%
  mutate(overall_fire_impact = factor(overall_fire_impact, levels = c("high", "med", "low", "zero"))) %>%
  select(-name) %>%
  mutate(`Bushfire Impact` = case_when(
    overall_fire_impact == 'high' ~ 'High',
  ...
  group_by(id, overall_fire_impact) %>%
  median_qi(value)

high <- predictions %>%
  filter(overall_fire_impact == 'high') %>%
  arrange(value)
...
zero <- predictions %>%
  filter(overall_fire_impact == 'zero') %>%
  arrange(value)
...
high %>%
  bind_rows(zero) %>%
  ggplot() + 
  geom_line(aes(value, ccdf, col = overall_fire_impact), size = 1) +
  scale_colour_manual(values = c("#B31529", "black")) +
  labs(y = "Probability of seeing phytoplankton counts at least this high", x= "Count", col = "Bushfire Impact") +
  theme_cowplot() + 
  theme(legend.position = 'none',
